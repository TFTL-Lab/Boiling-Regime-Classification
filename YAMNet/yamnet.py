...
#       outputs=[ embeddings, log_mel_spectrogram])
#   return frames_model
...
# contrastive learnig
def yamnet_with_embeddings(features, params):
    """
    Define the core YAMNet model that outputs embeddings instead of predictions
    """
    # Define the input and layers as you have it in the original yamnet function
    net = layers.InputLayer()(tf.reshape(features, (-1, 103, 98, 1)))  
    for (i, (layer_fun, kernel, stride, filters)) in enumerate(_YAMNET_LAYER_DEFS):
        net = layer_fun(f'layer{i + 1}', kernel, stride, filters, params)(net)
    
    embeddings = layers.GlobalAveragePooling2D()(net)  # This gives the embeddings
    return embeddings

def yamnet_with_embeddings_model(params):
    """
    Model that outputs embeddings (not predictions)
    """
    features = layers.Input(batch_shape=(None,), dtype=tf.float32)
    embeddings = yamnet_with_embeddings(features, params)
    model = Model(inputs=features, outputs=embeddings)
    return model